...
public class SystemProfiler implements ClassFileTransformer {
    final static String ROOT_PKG = "ru.progwards.java2.lessons".replace(".", "/"); // строка, указывающая на местонахождение тестируемого класса
    private static final ClassPool classPool = ClassPool.getDefault();
    private static final HashSet<String> inspectedClasses = new HashSet<String>(); // пул инспектируемых классов
    ...
    public byte[] transform(ClassLoader loader,
        ...
        if (className.startsWith(ROOT_PKG)) { // если адрес класса начинается с указанного местонахождения ("ru.progwards.java2.lessons")
            String shortName = className.substring(className.lastIndexOf("/") + 1); // берём только имя класса (то, что следует после последней "/")
            if (inspectedClasses.contains(shortName)) { // если этот класс есть в списке инспектируемых классов
                try {
